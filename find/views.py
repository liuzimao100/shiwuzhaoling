...
def compare_photo_with_database(photo_file):
    temp_path = None
    try:
        temp_dir = os.path.join(settings.MEDIA_ROOT, 'temp')
        os.makedirs(temp_dir, exist_ok=True)
        ...
        photo_lost_records = PhotoLost.objects.all()

        if not photo_lost_records.exists():
            return [0, "null"]

        best_match_path = None
        max_similarity = 0

        for record in photo_lost_records:
            db_img_path = os.path.join(settings.MEDIA_ROOT, record.image.name)
            ...
            try:
                ...
                if similarity > max_similarity:
                    max_similarity = similarity
                    best_match_path = record.image.name
        ...
        print(f"compare_photo_with_database函数出错: {str(e)}")
        import traceback
        traceback.print_exc()
        return [0, "null"]
    finally:
...
def upload_photo(request):
    if request.method != 'POST':
        return JsonResponse({'code': 400, 'message': '请求方法错误'}, status=400)

    if 'photo' not in request.FILES:
        return JsonResponse({'code': 400, 'message': '未上传照片'}, status=400)

    photo_file = request.FILES['photo']

    if not photo_file.name:
        return JsonResponse({'code': 400, 'message': '照片文件名为空'}, status=400)

    allowed_extensions = ['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp']
    file_ext = os.path.splitext(photo_file.name)[1].lower()
    if file_ext not in allowed_extensions:
        return JsonResponse({'code': 400, 'message': '不支持的图片格式'}, status=400)

    try:
        photo_lost = PhotoLost.objects.create(image=photo_file)
        return JsonResponse({
            'code': 200,
            'message': '上传成功',
            'data': {
        ...
        return JsonResponse({'code': 500, 'message': f'上传失败: {str(e)}'}, status=500)
...
def compare_photo(request):
    ...
    try:
        ...
        if result[0] == 1:
            ...
            return JsonResponse({'code': 400, 'message': '比对失败'})
    except Exception as e:
        print(f"比对照片失败: {str(e)}")
        return JsonResponse({'code': 500, 'message': f'比对失败: {str(e)}'}, status=500)